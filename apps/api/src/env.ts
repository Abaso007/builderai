...
export function createRuntimeEnv(workerEnv: Record<string, string | number | boolean | undefined>) {
  return createEnv({
    ...
    server: {
      ...
      CLOUDFLARE_LAKEHOUSE_ACCESS_KEY_ID: z.string().optional(),
      LAKEHOUSE_PIPELINE_USAGE: pipelinesBinding.optional(),
      LAKEHOUSE_PIPELINE_VERIFICATION: pipelinesBinding.optional(),
      LAKEHOUSE_PIPELINE_METADATA: pipelinesBinding.optional(),
      LAKEHOUSE_PIPELINE_ENTITLEMENT_SNAPSHOT: pipelinesBinding.optional(),
      LAKEHOUSE_STREAM_USAGE_URL: z.string().url().optional(),
      LAKEHOUSE_STREAM_VERIFICATIONS_URL: z.string().url().optional(),
      LAKEHOUSE_STREAM_METADATA_URL: z.string().url().optional(),
