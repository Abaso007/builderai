...
interface LakehouseObjectBucket {
  get(key: string): Promise<R2ObjectLike | null>
}

class BindingLakehousePipelineSender {
...
export class CloudflarePipelineLakehouseService implements LakehouseService {
  ...
  public async flushRaw(params: LakehouseFlushInput): Promise<LakehouseFlushResult> {
    try {
      ...
      const sourceBatches: Record<IndexedSource, unknown[]> = {
        ...
        verification: this.decorateRecords("verification", newVerificationRecords, ingestedAt),
      ...
      const nextVerificationId =
        newVerificationRecords.length > 0
          ? newVerificationRecords.reduce(
              (max, item) => Math.max(max, item.event_id),
              newVerificationRecords[0]!.event_id
            )
  ...
  public async getManifestFiles(params: LakehouseManifestQuery): Promise<LakehouseManifestFile[]> {
    this.logger.warn("Manifest file listing is not supported in pipeline mode", {
  ...
  public async listProjectsForDay(day: string): Promise<string[]> {
    this.logger.warn("Project listing by day is not supported in pipeline mode", {
      day,
    })
  ...
  public async compactDaySource(
    ...
    this.logger.info("Manual compaction skipped: managed by Iceberg maintenance", {
      projectId: params.projectId,
