/* eslint-disable */
// Generated by Wrangler by running `wrangler types` (hash: 264d1aecb6fe1e5bbaea0c0f53068393)
...
declare namespace Cloudflare {
	...
	type LakehouseVerificationsStreamDevRecord = { id: string; event_date: string | number; project_id: string; entitlement_id: string; feature_slug: string; customer_id: string; request_id: string; timestamp: string | number; schema_version: number; allowed?: boolean; denied_reason?: string; latency?: unknown; country?: string; region?: string; meta_id?: string; action?: string; key_id?: string; usage?: unknown; remaining?: number; unit_of_measure?: string; cost?: unknown; rate_amount?: unknown; rate_currency?: string };
	type LakehouseUsageStreamDevRecord = { id: string; event_date: string | number; request_id: string; project_id: string; customer_id: string; timestamp: number; idempotence_key: string; entitlement_id: string; feature_slug: string; schema_version: number; usage?: unknown; allowed?: boolean; deleted?: number; meta_id?: string; country?: string; region?: string; action?: string; key_id?: string; unit_of_measure?: string; cost?: unknown; rate_amount?: unknown; rate_currency?: string };
	type LakehouseMetadataStreamDevRecord = { id: string; event_date: string | number; project_id: string; customer_id: string; schema_version: number; timestamp: string | number; payload?: Record<string, unknown> };
	type LakehouseEntitlementsStreamDevRecord = { id: string; event_date: string | number; project_id: string; customer_id: string; schema_version: number; timestamp: string | number; feature_slug: string; feature_type: string; unit_of_measure: string; reset_config?: Record<string, unknown>; aggregation_method: string; merging_policy?: string; limit?: number; effective_at: string | number; expires_at?: string | number; version?: string; grants?: Record<string, unknown>; metadata?: Record<string, unknown> };
	interface ProdEnv {
		LAKEHOUSE: R2Bucket;
		LAKEHOUSE_BUCKET_NAME: "unprice-lakehouse-prod";
		LAKEHOUSE_ICEBERG_PREFIX: "__r2_data_catalog";
		...
		LAKEHOUSE_CREDENTIAL_TOKEN: string;
		DATABASE_URL: string;
	...
	interface PreviewEnv {
		LAKEHOUSE: R2Bucket;
		LAKEHOUSE_BUCKET_NAME: "unprice-lakehouse-preview";
		LAKEHOUSE_ICEBERG_PREFIX: "__r2_data_catalog";
		TINYBIRD_TOKEN: string;
		TINYBIRD_URL: string;
		CLOUDFLARE_ZONE_ID: string;
	...
	interface DevEnv {
		...
		RL_FREE_1000_60s: RateLimit;
		RL_FREE_6000_60s: RateLimit;
		PIPELINE_VERIFICATIONS: import("cloudflare:pipelines").Pipeline<Cloudflare.LakehouseVerificationsStreamDevRecord>;
		PIPELINE_USAGE: import("cloudflare:pipelines").Pipeline<Cloudflare.LakehouseUsageStreamDevRecord>;
		PIPELINE_METADATA: import("cloudflare:pipelines").Pipeline<Cloudflare.LakehouseMetadataStreamDevRecord>;
		PIPELINE_ENTITLEMENTS: import("cloudflare:pipelines").Pipeline<Cloudflare.LakehouseEntitlementsStreamDevRecord>;
	}
	interface Env {
		TINYBIRD_TOKEN: string;
		TINYBIRD_URL: string;
...
declare namespace NodeJS {
	interface ProcessEnv extends StringifyValues<Pick<Cloudflare.Env, "LAKEHOUSE_BUCKET_NAME" | "LAKEHOUSE_ICEBERG_PREFIX" | "TINYBIRD_TOKEN" | "TINYBIRD_URL" | "CLOUDFLARE_ZONE_ID" | "CLOUDFLARE_API_TOKEN" | "CLOUDFLARE_ACCOUNT_ID" | "CLOUDFLARE_API_TOKEN_LAKEHOUSE" | "CLOUDFLARE_LAKEHOUSE_ACCESS_KEY_ID" | "LAKEHOUSE_CREDENTIAL_TOKEN" | "DATABASE_URL" | "DATABASE_READ1_URL" | "DATABASE_READ2_URL" | "ENCRYPTION_KEY" | "AUTH_SECRET" | "DRIZZLE_LOG" | "APP_ENV" | "NODE_ENV" | "AXIOM_API_TOKEN" | "AXIOM_DATASET" | "UPSTASH_REDIS_REST_URL" | "UPSTASH_REDIS_REST_TOKEN" | "UNPRICE_API_KEY" | "UNPRICE_API_URL">> {}
}
declare module "*.sql" {
	const value: string;
